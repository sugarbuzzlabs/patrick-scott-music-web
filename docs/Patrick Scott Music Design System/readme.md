...
- **Logo:** mono-stamp wordmark — `PATRICK SCOTT/MUSIC` set in IBM Plex Mono 400, 0.08em tracking, with the slash in gold (`--text-accent`). Stacked (primary), horizontal (nav/footer), and `PS/M` compact mark for avatars. See `guidelines/brand-logo.html` and `assets/logo*.svg`. Prefer rendering it as live HTML text (the SVGs fall back to system monospace inside `<img>`). is made later, replace this.
...
- `templates/website/` — "Artist site" template (homepage) for consuming projects
...
- `assets/fonts/` — self-hosted Archivo (variable, wdth+wght) and IBM Plex Mono (400/500/600–700), declared in `tokens/fonts.css`
- `SKILL.md` — agent skill entry point

**Caveats:** Logo is a commissioned typographic stamp (see ICONOGRAPHY); no externally supplied mark exists.
